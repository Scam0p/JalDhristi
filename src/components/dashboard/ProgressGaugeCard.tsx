...
export const ProgressGaugeCard: React.FC<ProgressGaugeCardProps> = ({ kpis, currentCase }) => {
  // Wait reduction percentage compared to manual baseline (11.4 min)
  const reductionPct = currentCase === 'ai' ? 54 : currentCase === 'conventional' ? 29 : 0;
  const displayPercentage = currentCase === 'ai' ? '54%' : currentCase === 'conventional' ? '29%' : '0%';
  ...
  return (
    <div className="donezo-card p-5 flex flex-col justify-between h-full">
      ...
      {/* Semi-Circular Progress Gauge (Inspired directly by reference '41% Project Ended') */}
      <div className="relative flex flex-col items-center justify-center my-2">
        <svg className="w-48 h-28" viewBox="0 0 200 110">
          ...
          <path
            ...
            strokeDashoffset={251.2 - (251.2 * Math.max(0.08, reductionPct / 100))}
      ...
      {/* Bottom Legend (Dots like reference) */}
